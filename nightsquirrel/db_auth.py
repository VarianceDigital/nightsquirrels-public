...
def db_update_user_key(usr_id, access_key):
    db = get_db()
    cur = db.cursor(cursor_factory=RealDictCursor)
    hash = generate_password_hash(access_key, "pbkdf2:sha256")
    cur.execute("""
                    UPDATE nightsquirrel.tbl_u_user
	                set usr_key=%s, usr_key_temp=%s
	                WHERE usr_id=%s
                    """, 
                    (hash, access_key, usr_id, )
    )
...
def db_check_user(form_data):
    ...
    cur.execute("""
                SELECT * FROM nightsquirrel.tbl_u_user
                WHERE usr_email=%s 
                """ , 
                (email,)
    )
    ...
    if user:
        if check_password_hash(user['usr_key'], key):
            return user
...
def db_check_if_email_exists(email):
    ...
    cur.execute(
        'SELECT * FROM nightsquirrel.tbl_u_user WHERE usr_email=%s', (email,)
    )
...
def db_check_otp(usr_id, otp):
    ...
    cur.execute(
        'SELECT * FROM nightsquirrel.tbl_u_user WHERE usr_id=%s AND usr_otp=%s', 
        (usr_id, otp,)
    )
...
def db_check_username_unique(user_name):
    ...
    cur.execute("""SELECT * FROM nightsquirrel.tbl_u_user
	                WHERE usr_name=%s
                    """, 
                    (user_name,)
    )
...
def db_create_user_entry(email, access_key, custom_name, tile_filename):
    ...
    cur.execute("""
	                ...
	                usr_email, usr_key, usr_key_temp, usr_name, usr_tile)
	                VALUES (%s, %s, %s,%s,%s)
                    RETURNING usr_id;
                    """, 
...
def db_get_user_by_id(usr_id):
    """
    Helper used by Flask-Login to load a user by ID.
    Returns a dict row from nightsquirrel.tbl_u_user or None.
    """
    if usr_id is None:
        return None
...
def db_get_user_by_email(email):
    """Look up a valid user by email. Returns a dict row or None."""
    if not email:
        return None
...
def db_set_user_as_payer(usr_id):
    """Mark a user as a payer (set usr_is_payer = true).
    Safe to call on any user, including students (self-pay).
    """
    db = get_db()
    cur = db.cursor()
    cur.execute("""
        UPDATE nightsquirrel.tbl_u_user
        SET usr_is_payer = true
        WHERE usr_id = %s
    """, (usr_id,))
    db.commit()
    cur.close()
...
def db_set_user_as_payer_only(usr_id):
    """Configure a newly created user as a payer-only account:
    set usr_is_payer = true and usr_is_student = false.
    Used for new payer accounts that should have no student/tutor/admin flags.
    """
    db = get_db()
    cur = db.cursor()
    cur.execute("""
        UPDATE nightsquirrel.tbl_u_user
        SET usr_is_student = false,
            usr_is_payer = true
        WHERE usr_id = %s
    """, (usr_id,))
    db.commit()
...
def db_update_user_name(usr_id, usr_name):
    db = get_db()
    cur = db.cursor(cursor_factory=RealDictCursor)
...
def db_update_user(usr_id, email, access_key):
    db = get_db()
    cur = db.cursor(cursor_factory=RealDictCursor)
    hash = generate_password_hash(access_key, "pbkdf2:sha256")
    cur.execute("""
                    UPDATE nightsquirrel.tbl_u_user
	                set usr_email=%s, usr_key=%s, usr_key_temp=%s, usr_confirmed=false
	                WHERE usr_id=%s
                    """, 
                    (email, hash, access_key, usr_id, )
    )
...
def db_update_student_academic_profile(usr_id, sct_id, school_grade):
    """Update the academic profile fields for a student."""
    db = get_db()
    cur = db.cursor(cursor_factory=RealDictCursor)
    cur.execute("""
        UPDATE nightsquirrel.tbl_u_user
        SET sct_id = %s, usr_school_grade = %s
        WHERE usr_id = %s
    """, (sct_id, school_grade, usr_id))
    db.commit()
    cur.close()
...
def create_email_link_token(new_id, email, secret):
    ...
    payloadJWT={}
    payloadJWT["usr_id"]=new_id
    payloadJWT["usr_email"]=email
    payloadJWT["usr_random_key"]=create_random_access_key() #<-- stir things up
...
def db_check_email_link_token(token,secret):
    ...
    cur.execute("""SELECT usr_id, usr_email
                    FROM nightsquirrel.tbl_u_user
                    WHERE usr_id = %s AND usr_email= %s""", (usr_id, usr_email,)
    )
...
def db_delete_account(usr_id):
    db = get_db()
    cur = db.cursor(cursor_factory=RealDictCursor)

    #GET THE USER TILE
    cur.execute("""SELECT usr_tile FROM nightsquirrel.tbl_u_user
	                WHERE usr_id=%s
                    """,
                    (usr_id,)
    )
    tileToBeRemoved_rec = cur.fetchone()
    tileToBeRemoved = tileToBeRemoved_rec['usr_tile']

    # Delete payment records where this user is the payer (ON DELETE RESTRICT)
    cur.execute("""DELETE FROM nightsquirrel.tbl_p_payment
                    WHERE payer_usr_id=%s
                    """,
                    (usr_id,)
    )

    # Delete questions owned by this user (cascades to tickets, answers, payments-by-ticket)
    cur.execute("""DELETE FROM nightsquirrel.tbl_q_question
                    WHERE usr_id=%s
                    """,
                    (usr_id,)
    )

    cur.execute("""DELETE FROM nightsquirrel.tbl_u_user
	                WHERE usr_id=%s
                    """,
                    (usr_id,)
    )

    # Remaining related entities (payer_link, payment_method) are deleted
    # automatically via ON DELETE CASCADE constraints
...
def db_create_custom_tile(custom_color):
    ...
    allcolors = list(map(lambda x: re.findall(r'#\w+;',x)[0], fills))
